...
// FUNCTIONS //
void cmd_Home(char *command, char cState)
{
	switch(cState)
	{
		case '1':
			strcpy(command, HOME_COMMAND);
			break;
...
void cmd_Motor(char *command, char *sAxis)
{
	int i = 0;
	...
	sprintf(command, "$M%03d%c%03d%c%%", iRMotor, cRMDir, iLMotor, cLMDir);
...
void cmd_Camera(char *command, char *sAxis)
{
	int i = 0;
